...
if needToMask
    ...
    colormapeditor;
    display('Adjust colormap to mask red areas correctly and press any key. P.S. the red area represent the area that will be cropped out in the next step. Before pressing enter, make sure to go back and forth of the window to make sure the red area is correctly selected')
    pause on
    pause
    cmap = colormap;
...
if needToDoSections
    ...
    for i = 1:size(sections,1)
        for j = 1:size(sections,2)
            ...
            %static section
            %sections{i,j}.moving.windowed =
            %histeq(sections{i,j}.moving.windowed,hgram); <- this line is
            %disable. Enabling this line will bring pixel values from
            %post-expansion to pre-expansion
